...
import { pagesToShowInitially } from '../../../utils/constants';
...
const GuestsList = () => {
  ...
  // Fetch guests using your guestservice
  useEffect(() => {
    getguests();
  }, [currentPage]);
  ...
  const getguests = async () => {
    setLoading(true);
    try {
      const response = await BookingService.getguests({ page: currentPage,
        limit: itemsPerPage});
      console.log(response)
      if (response) {
        setguests(response.details); // Set categories data to states
  ...
  const filteredData = useMemo(() => {
    return guests.filter((guests) =>
      guests?.first_name?.toLowerCase()?.includes(searchInput?.toLowerCase())
    );
  }, [guests, searchInput]);

  // Navigate to the Edit page for a given booking
  const gotoView = (booking) => {
    navigate(`/admin/guests/view/${booking._id}`);
  };
  ...
  return (
    <Box>
      ...
      <Card>
        <Box>
          <Flex p="5">
            <Text p="2" mb={2}>Search by Guest Name:</Text>
            <Input
              placeholder="Search"
              value={searchInput}
            ...
            <Table variant="striped" colorScheme="gray">
              ...
              <Tbody>
                ...
                {(currentPage - 1) * itemsPerPage + index + 1}
                    ...
                    <Td textAlign="center">{guest.first_name}</Td>
                    <Td textAlign="center">{guest?.mobile_phone?.number}</Td>
                                       <Td textAlign="center">{guest?.gender ==1 ?"Male":"Female"}</Td>
                    <Td textAlign="center">
